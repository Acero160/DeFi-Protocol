...
contract DSCEngineTest is StdCheats, Test {
    ...
    function testRevertsIfTransferFromFails() public {
        ...
        vm.prank(owner);
        MockFailedTransferFrom mockCollateralToken = new MockFailedTransferFrom(address(this));
   ...
   function testRevertsIfTransferFails() public {
    // Arrange - Setup
    address owner = address(this); // Este contrato es el owner
    vm.startPrank(owner); // Simula que todo lo hace el owner

    MockFailedTransfer mockDsc = new MockFailedTransfer(owner); // owner correcto
    tokenAddresses = [address(mockDsc)];
    feedAddresses = [ethUsdPriceFeed];
    DSCEngine mockDsce = new DSCEngine(tokenAddresses, feedAddresses, address(mockDsc));
    mockDsc.mint(user, amountCollateral);

    mockDsc.transferOwnership(address(mockDsce)); // Ya somos el owner, no hay error
    vm.stopPrank(); // Fin de acciones como owner
    ...
    function testMustImproveHealthFactorOnLiquidation() public {
        // Arrange - Setup
        MockMoreDebtDSC mockDsc = new MockMoreDebtDSC(ethUsdPriceFeed, address(this));
        tokenAddresses = [weth];
        feedAddresses = [ethUsdPriceFeed];
        address owner = msg.sender;
        vm.prank(owner);
